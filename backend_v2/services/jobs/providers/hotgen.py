...
from services.hotgen_provider import create_task as create_hotgen_task
from services.hotgen_provider import get_task as get_hotgen_task
from services.hotgen_provider import refresh_task_account
from services.hotgen_provider import release_task_account
...
class HotgenAdapter(ProviderAdapter):
    name = "hotgen"
    ...
    def run(self, job: dict[str, Any], store: JobStore) -> list[dict[str, Any]]:
        ...
        submitted = create_hotgen_task(payload)
        if not submitted.get("success"):
            message = (submitted.get("error") or {}).get("message") or "Hotgen submit failed"
            raise ProviderError(message)

        task_id = self._extract_task_id(submitted)
        if not task_id:
            raise ProviderError("Hotgen did not return task_id")
        ...
        try:
            ...
            while True:
                ...
                if time.time() - started_at > self.timeout_seconds:
                    raise ProviderTimeout(f"Hotgen task timeout after {self.timeout_seconds}s")

                status_payload = get_hotgen_task(task_id)
                ...
                if status in TERMINAL_FAILED:
                    message = (status_payload.get("error") or {}).get("message") or "Hotgen task failed"
                    self._refresh_account_snapshot(task_id)
                    raise ProviderError(message)
