...
class HotgenAccountPool:
    ...
    def __init__(self, *, per_token_limit: int = DEFAULT_PER_TOKEN_LIMIT):
        ...
        self._last_skip_reasons: list[str] = []
        self._cursor = 0
        ...
        *,
        ...
        for index, token in enumerate(ordered_tokens):
            ...
            if digest in excluded:
                skip_reasons.append(f"{masked}: already attempted")
                continue
            if token in self._disabled:
                skip_reasons.append(f"{masked}: disabled")
                continue
            cooldown = self._cooldowns.get(digest)
            if cooldown and float(cooldown.get("until") or 0) > now:
                skip_reasons.append(f"{masked}: cooldown {cooldown.get('error') or 'temporarily unavailable'}")
                continue
            local_count = len(self._in_flight.get(digest) or [])
            if local_count >= self.per_token_limit:
                skip_reasons.append(f"{masked}: local running {local_count} >= limit {self.per_token_limit}")
                continue

            record = self._records_by_hash.get(digest)
            if record and record.get("error"):
                skip_reasons.append(f"{masked}: cached error {record.get('error')}")
                continue
            ...
            if total_credit is not None and total_credit < threshold:
                skip_reasons.append(f"{masked}: credit {total_credit:g} < threshold {threshold}")
                continue
            if total_credit is not None and total_credit < required_credits:
                skip_reasons.append(f"{masked}: credit {total_credit:g} < required {required_credits}")
                continue
        ...
        self._last_skip_reasons = skip_reasons[-6:]
        return candidates
    ...
    def _no_account_message_locked(self, required_credits: int, now: float) -> str:
        ...
        details = self._last_skip_reasons or []
        if active_cooldowns:
            details.append(f"recent unavailable token: {active_cooldowns[-1]}")
        suffix = f" Details: {'; '.join(details)}" if details else ""
        return f"No available Hotgen account for {required_credits} credits.{suffix}"

    def _reload_if_needed_locked(self) -> None:
        config_mtime = _mtime(config.HOTGEN_CONFIG_PATH)
        config_changed = config_mtime != self._config_mtime
        if config_changed:
            payload = _read_json(config.HOTGEN_CONFIG_PATH)
...
def _accounts_path(payload: dict[str, Any]) -> str:
    ...
    path = payload.get("accounts_path") or "hotgen_accounts.json"
...
ACCOUNT_POOL = HotgenAccountPool()
